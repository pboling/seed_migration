module DataMigration
    class Migrator
        ...
        def self.migration_path(filename)
            if Rails.env.test?
                return Rails.root.join("spec", "harrys_data", "fixtures").join(filename).to_s
            end
        ...
        def up
            ...
            raise "#{klass} has already been migrated." if DataMigration.where(version: version).first
        ...
        def down
            ...
            migration = DataMigration.where(version: version).first
        ...
        def self.bootstrap
            files = HarrysData::Migrator.get_migration_files
            files.each do |file|
        ...
        def self.get_new_migrations
            ...
            all_migration_versions = DataMigration.all.map(&:version)
        ...
        def self.get_last_x_migrations(x = 1)
            # Grab data from DB
            migrations = DataMigration.order("version DESC").limit(x).pluck("version")
        ...
        def self.get_last_migration_date
            return nil if DataMigration.count == 0
            DateTime.parse(DataMigration.maximum("version"))
        end

        def self.get_migration_files
            files = Dir.glob(HarrysData::Migrator.migration_path("*_*.rb"))
